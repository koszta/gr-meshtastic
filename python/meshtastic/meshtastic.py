...
def to_simulation_packet(packet, key):
    ...
    meshPacket = mesh_pb2.MeshPacket(
        ...
        decoded=mesh_pb2.Data(
            payload=decrypt(packet, key), portnum=portnums_pb2.SIMULATOR_APP
        ),
    )
...
class meshtastic(gr.sync_block):
    ...
    def __init__(self, key="1PG7OiApB1nwvP+rz05pAQ==", url="http://localhost"):
        gr.sync_block.__init__(
            self, name="Meshtastic Simulator", in_sig=[np.byte], out_sig=[]
        )
    ...
    def work(self, input_items, output_items):
        try:
            requests.put(
                self.url + "/api/v1/toradio",
                data=to_simulation_packet(
                    input_items[0].tobytes(), self.key
                ).SerializeToString(),
